...
class YOLO(object):
    ...
    def detect_image(self, image, crop=False, count=False):
        ...
        disparity_valid = disparity[mask]

        if disparity_valid.size > 0:
            # 设置有效视差范围
            valid_disp_min = np.percentile(disparity_valid, 1)
            valid_disp_max = np.percentile(disparity_valid, 99)

            # 剔除异常值
            disparity[disparity < valid_disp_min] = valid_disp_min
            disparity[disparity > valid_disp_max] = valid_disp_max
        ...
        for i, c in list(enumerate(top_label)):
            ...
            top = max(0, np.floor(top).astype('int32'))
            left = max(0, np.floor(left).astype('int32'))
            bottom = min(480, np.floor(bottom).astype('int32'))  # 使用固定高度 480
            right = min(640, np.floor(right).astype('int32'))  # 使用固定宽度 640

            # 计算中心点坐标并确保在有效范围内
            middle_x = min(max(0, int(np.floor((left + right) / 2))), 639)
            middle_y = min(max(0, int(np.floor((top + bottom) / 2))), 479)

            print('\n像素坐标 x = %d, y = %d' % (middle_x, middle_y))

            # 确保 threeD 数组存在且维度正确
            if threeD is not None and threeD.shape[0] >= 480 and threeD.shape[1] >= 640:
                try:
                    world_x = threeD[middle_y][middle_x][0] / 1000.0
                    world_y = threeD[middle_y][middle_x][1] / 1000.0
                    world_z = threeD[middle_y][middle_x][2] / 1000.0
            ...
            label_size = np.array([bbox[2] - bbox[0], bbox[3] - bbox[1]])
